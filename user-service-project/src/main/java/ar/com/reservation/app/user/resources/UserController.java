package ar.com.reservation.app.user.resources;

import ar.com.reservation.app.user.domain.model.entity.Entity;
import ar.com.reservation.app.user.domain.model.entity.User;
import ar.com.reservation.app.user.domain.service.UserService;
import ar.com.reservation.app.user.domain.valueobject.UserVO;
...
public class UserController {
    ...
    public ResponseEntity<User> add(@RequestBody UserVO userVO) {
        logger.info(String.format("user-service add() invoked: %s for %s",
    ...
    public ResponseEntity<Collection<User>> defaultUsers(String input) {
        logger.warning("Fallback method for user-service is being used.");
        return new ResponseEntity<>((Collection<User>) null, HttpStatus.NO_CONTENT);
    }
